  ...
  default:
    if (std::isalpha(source[current_index])) {
      ...
      if (result->value == "true") {
        std::unique_ptr<Boolean> result = std::make_unique<Boolean>(Boolean(true));
        return result;
      }
      else if (result->value == "false") {
        std::unique_ptr<Boolean> result = std::make_unique<Boolean>(Boolean(false));
        return result;
      }

      return result;
    } else if (std::isdigit(source[current_index]) ||
               source[current_index] == '-' || source[current_index] == '+') {
